...
Recent changes (fix all architectural issues):
  [fix] asyncio.Lock for acquire_async (replaces threading.Lock)
  [fix] asyncio.open_connection for port scanner (no threads)
  [fix] asyncio.to_thread for SQLite I/O (non-blocking)
  [fix] async_playwright when available (true non-blocking SPA crawl)
  [fix] aiodns resolver reuse + DNS rate limiter (semaphore)
  [fix] IDOR: field-level verification, not just body similarity
  [fix] SQLi time-based: statistical baseline timing
  [fix] XXE: multipart/form-data + file upload vectors
  [fix] html.escape() on all HTML report output
  [fix] User-Agent rotation + jitter on rate limiter
  [fix] --aggressive flag for dangerous probes (SSRF/XXE/file)
  [fix] --debug flag for stack traces & verbose logging
  [fix] --waf-bypass flag: force bypass even when no WAF detected
  [fix] --no-verify flag for SSL verification control
  [fix] JWT disclaimer: explicit warning "claims only, no sig verify"
...
Dependencies:
  ...
  pip install aiodns          # optional, async DNS
...
class ScanResult:
    ...
    def save_checkpoint_sqlite(self, db_path: str):
        ...
        # FIX 4: Scan queue table — persistent queue for optimal resume
        conn.execute("""
    ...
    def load_checkpoint_sqlite(cls, db_path: str) -> Optional["ScanResult"]:
        ...
        try:
            ...
            # FIX 4 (read side): reload unprocessed URLs from the queue
            try:
                ...
                result.scan_queue_urls = []  # table not present in older checkpoints
            conn.close()
            return result
        except Exception as e:
...
class RateLimiter:
    ...
    def __init__(self, rps: float = 10.0):
        ...
        # FIX 1: asyncio.Lock created in __init__ — no lazy init,
        # no race condition if two coroutines enter simultaneously
    ...
    async def acquire_async(self):
        """
        FIX: Use asyncio.Lock (not threading.Lock) to avoid ever blocking
        the event loop. Random jitter prevents predictable request patterns
        that could be detected or blocked by the target's rate limiter.
        """
        while True:
            ...
            if wait <= 0:
                # Jitter: add a small random delay
...
# ─────────────────────────────────────────────
#  USER-AGENT POOL (rotation per request)
# ─────────────────────────────────────────────
UA_POOL = [
...
class AsyncEngine:
    ...
    def __init__(self, max_workers: int = 20, rps: float = 10.0,
        ...
        self.cacert       = cacert
        # FIX 14: Bearer token — injected into every request
    ...
    def _headers(self, extra: dict = None) -> dict:
        """UA rotation — each request uses a different browser string.
        FIX 14: Inject Bearer token if available."""
    ...
    async def detect(self, engine: AsyncEngine, url: str,
        ...
        if waf_info["detected"]:
            result.add(Finding(WAFDetector.NAME, Severity.INFO,
                f"WAF Detected: {waf_info['name']}",
                "WAF is active — payloads will be mutated automatically.",
                f"WAF: {waf_info['name']} | Confidence: {waf_info['confidence']}%\n"
                f"Bypass: {waf_info['bypass_strategy']}",
                "A WAF is not a reason to skip fixing vulnerabilities.",
                0,"",url,["waf"]))
        return waf_info
...
class SPACrawler:
    """
    FIX: Use async_playwright when available (true non-blocking).
    Falls back to sync_playwright in a thread if async is unavailable.
    """
    NAME = "SPA Crawler"
    PAGE_TIMEOUT    = 12000
    ...
    async def run_async(self, url: str, result: ScanResult,
        ...
        if not PLAYWRIGHT_OK:
            result.errors.append("SPA Crawler: Playwright not available")
            return
        if PLAYWRIGHT_ASYNC:
    ...
    async def _crawl_async(self, url: str, result: ScanResult,
                           js_events: bool):
        """True async crawl via playwright.async_api — non-blocking."""
    ...
    def _crawl_sync(self, url: str, result: ScanResult, js_events: bool):
        """Fallback: sync_playwright in a separate thread."""
    ...
    def _add_findings(self, result, url, api_calls, found_urls):
        if api_calls:
            result.add(Finding(self.NAME, Severity.INFO,
                f"SPA: {len(api_calls)} API Endpoints Detected",
                f"Playwright intercepted {len(api_calls)} XHR/Fetch calls.",
                "Sample:\n"+"\n".join(
        ...
        if found_urls:
            result.add(Finding(self.NAME, Severity.INFO,
                f"SPA: {len(found_urls)} URLs Discovered",
                "Crawler found URLs including SPA routes.",
                "Sample:\n"+"\n".join(list(found_urls)[:8]),
                "Review all discovered endpoints.",0,"",url))
...
class NucleiEngine:
    ...
    async def _run_subprocess(self, result, url):
        if not self.template_dir:
            result.errors.append("Nuclei: template directory not found")
            return
...
class AttackChainReporter:
    ...
    CHAINS = [
        {"id":"chain-001","name":"Path Traversal → Secret Leak → Session Risk",
         ...
         "description":"Sensitive file exposed + credentials leaked + insecure cookies.",
        ...
        {"id":"chain-002","name":"XSS → CSRF Bypass → Privilege Escalation",
         ...
         "description":"XSS + no CSRF protection + no rate limiting.",
         "impact":"CSRF via XSS"},
        {"id":"chain-003","name":"SQLi → Data Dump → Auth Bypass",
         ...
         "description":"SQLi + exposed database + weak security headers.",
        ...
        {"id":"chain-006","name":"SSRF → Cloud Metadata → Takeover",
         ...
         "description":"SSRF to cloud metadata + IDOR → credential exfiltration.",
        ...
        {"id":"chain-007","name":"JWT Weak Secret → Auth Bypass",
         ...
         "severity":Severity.CRITICAL,"cvss":9.5,
         "description":"JWT weak secret / alg:none + IDOR → admin takeover.",
    ...
    def report(self, result: ScanResult):
        ...
        result.add(Finding(self.NAME, Severity.CRITICAL,
            f"Attack Chain Detected: {len(chains)} Scenario(s)",
            "Vulnerabilities can be chained together into a multi-step attack.",
            chain_summary,
            "Prioritize fixing findings that are part of an attack chain.",
...
class AsyncHeaderAnalyzer(AsyncModule):
    ...
    REQUIRED = {
        ...
        "Content-Security-Policy":  (Severity.HIGH,6.1,"CWE-693",
            "CSP header is missing.","Add a Content-Security-Policy header."),
        ...
        "X-Content-Type-Options":   (Severity.MEDIUM,3.7,"CWE-430",
            "MIME sniffing is enabled.","X-Content-Type-Options: nosniff"),
        "Referrer-Policy":          (Severity.LOW,2.6,"CWE-200",
            "URL may leak to third parties.",
            "Referrer-Policy: strict-origin-when-cross-origin"),
        "Permissions-Policy":       (Severity.LOW,2.1,"CWE-269",
            "Browser features are unrestricted.","Add a Permissions-Policy header."),
    }
    LEAK = {"Server":"Server version exposed","X-Powered-By":"Backend stack exposed",
            "X-AspNet-Version":"ASP.NET version exposed","X-Runtime":"Runtime version exposed"}
    ...
    async def run_async(self, engine, result, url, waf_info, aggressive=False):
        ...
        for hdr,(sev,cvss,cwe,desc,fix) in self.REQUIRED.items():
            if hdr.lower() not in hdrs_l:
                result.add(Finding(self.NAME,sev,f"Missing Header: '{hdr}'",
                    desc,f"Header not found on {url}",fix,
        ...
        for hdr,desc in self.LEAK.items():
            ...
            if v:
                result.add(Finding(self.NAME,Severity.LOW,
                    f"Information Exposed: {hdr}",desc,f"{hdr}: {v}",
                    f"Remove the '{hdr}' header.",3.1,"CWE-200",url,
                    ["info","headers"]))
        ...
        if rt and rf:
            ...
            if sbt > 0.85 and stf < 0.75:
                result.add(Finding(self.NAME,Severity.HIGH,
                    ...
                    f"TRUE/FALSE responses differ (sim={stf:.3f}).",
                    f"URL: {t_url}\nParams: {list(params.keys())}\n"
                    f"Sim TRUE/FALSE: {stf:.3f}",
                    "Use Prepared Statements.",7.5,"CWE-89",t_url,["sqli"]))
                return
        ...
        async def test_pl(payload, technique, cvss):
            ...
            for variant in variants:
                ...
                if any(x in payload.upper() for x in ["SLEEP","PG_SLEEP"]):
                    # FIX: statistical timing — take 3x baseline, then compare
                    if elapsed >= 1.8:
                        # Verify with a re-test: check whether a non-sleep request
                        # is also slow (network latency issue)
                        ...
                        # Only report if payload delay > 2.5x baseline delay
            ...
            async def test_pl(payload, ptype):
                ...
                for variant in variants:
                    ...
                    if "{{7*7}}" in payload and "49" in r2["body"] and "49" not in bl:
                        result.add(Finding(self.NAME,Severity.CRITICAL,
                            "SSTI — Potential RCE!","{{7*7}}→49.",
                            f"URL: {t_url}\nPayload: {{{{7*7}}}}",
                            "Sanitize template input.",9.8,"CWE-94",t_url,["ssti","xss"]))
                        return
                    d       = Sim.diff(bl, r2["body"], payload=variant)
                    ...
                    if d["reflected"] and not escaped and \
                        ...
                        result.add(Finding(self.NAME,Severity.HIGH,
                            ...
                            "Encode output. Apply a Content Security Policy.",
...
class AsyncSensitiveFiles(AsyncModule):
    ...
    async def run_async(self, engine, result, url, waf_info, aggressive=False):
        ...
        # FIX 10: Combine static path list + paths discovered by crawler.
        # Extract unique paths from result.endpoints (without domain/query).
        ...
        for ep in result.endpoints:
            try:
                ...
                if p and p not in self.paths:
                    extra_paths.add(p)
                    # Also try simple path traversal variants
                    if "/" in p.strip("/"):
        ...
        async def check(path):
            ...
            result.add(Finding(self.NAME, sev, f"Sensitive Path: {path}",
                "Sensitive file or directory is publicly accessible.",
                f"GET {full} → {resp['status']} ({len(resp['body'])} bytes)",
                f"Restrict access to {path}.", cvss, "CWE-538", full, ["files"]))
        ...
        async def check(origin):
            ...
            if acao == "*":
                result.add(Finding(self.NAME,Severity.MEDIUM,"CORS Wildcard",
                    "All origins are allowed.","ACAO: *",
                    "Whitelist specific origins.",5.4,"CWE-942",url,["cors"]))
            elif acao == origin and acac.lower() == "true":
                result.add(Finding(self.NAME,Severity.HIGH,
                    "CORS: Origin Reflection + Credentials",
                    "Origin is reflected and credentials are allowed.",
                    f"Origin: {origin}\nACAO: {acao}\nACAC: {acac}",
                    "Strictly validate the origin.",8.1,"CWE-942",url,["cors"]))
...
class AsyncInfoDisclosure(AsyncModule):
    ...
    async def run_async(self, engine, result, url, waf_info, aggressive=False):
        resps = await asyncio.gather(
            ...
            return_exceptions=True)
        # FIX 5: Log exceptions from gather — do not silently discard them
...
# FIX: asyncio.open_connection — replaces ThreadPoolExecutor
class AsyncPortScanner(AsyncModule):
    NAME = "Port Scanner"
    PORTS = {
        21:("FTP",Severity.HIGH,"FTP is unencrypted"),
        22:("SSH",Severity.INFO,"SSH is open"),
        23:("Telnet",Severity.CRITICAL,"Telnet is plaintext!"),
        80:("HTTP",Severity.INFO,"HTTP is open"),
        443:("HTTPS",Severity.INFO,"HTTPS is open"),
        1433:("MSSQL",Severity.HIGH,"MSSQL is exposed"),
        2375:("Docker",Severity.CRITICAL,"Docker API without TLS!"),
        3306:("MySQL",Severity.HIGH,"MySQL is exposed"),
        3389:("RDP",Severity.HIGH,"RDP is exposed"),
        5432:("PostgreSQL",Severity.HIGH,"PostgreSQL is exposed"),
        6379:("Redis",Severity.CRITICAL,"Redis has no authentication!"),
        8080:("HTTP-Alt",Severity.MEDIUM,"HTTP alternate port"),
        8888:("Jupyter",Severity.CRITICAL,"Jupyter Notebook exposed!"),
        9200:("Elastic",Severity.CRITICAL,"Elasticsearch has no authentication!"),
        27017:("MongoDB",Severity.CRITICAL,"MongoDB has no authentication!"),
    }

    async def run_async(self, engine, result, url, waf_info, aggressive=False):
        host = urllib.parse.urlparse(url).netloc.split(":")[0]

        # FIX 2: Adaptive timeout — common ports get more time, others less
        custom_timeout = getattr(engine, '_port_timeout', None)

        async def scan_async(port) -> Tuple[int, bool]:
            """
            FIX: asyncio.open_connection — lightweight, true async.
            Adaptive timeout: 2s for common ports, 1s for others.
            --port-timeout overrides all.
            """
        ...
        for _item in results:
            ...
            if open_:
                ...
                result.add(Finding(self.NAME,sev,f"Port {port}/{svc} Open",
                    desc,f"host={host} port={port} OPEN",
                    f"Close port {port} via firewall.",cvss,"CWE-284",
                    f"{host}:{port}",["ports"]))
        ...
        for name in resp["cookies"]:
            ...
            if "httponly" not in raw:
                result.add(Finding(self.NAME,sev,
                    ...
                    "Add the HttpOnly flag.",4.3,"CWE-1004",url,["cookies"]))
            if url.startswith("https://") and "secure" not in raw:
                result.add(Finding(self.NAME,sev,
                    f"Cookie '{name}' Missing Secure Flag","Can be sent over HTTP.",
                    f"Set-Cookie: {name} (no Secure)",
                    "Add the Secure flag.",4.3,"CWE-614",url,["cookies"]))
            if "samesite" not in raw:
                result.add(Finding(self.NAME,Severity.LOW,
                    f"Cookie '{name}' Missing SameSite","Vulnerable to CSRF.",
                    f"Cookie {name} — SameSite is not set.",
                    "Add SameSite=Strict.",3.1,"CWE-352",url,
                    ["cookies","auth"]))
        ...
        async def check(path):
            ...
            if not any(x in body for x in ["captcha","recaptcha","hcaptcha"]):
                result.add(Finding(self.NAME,Severity.MEDIUM,
                    f"Login Without CAPTCHA: {path}","Vulnerable to automated login.",
                    f"URL: {base+path}",
                    "Implement CAPTCHA.",5.3,"CWE-307",base+path,["auth"]))
            if not any(x in body for x in
                       ["csrf","_token","authenticity_token"]):
                result.add(Finding(self.NAME,Severity.HIGH,
                    f"Login Without CSRF Token: {path}","Vulnerable to CSRF.",
                    "No CSRF token found.",
                    "Implement CSRF tokens.",6.5,"CWE-352",base+path,
                    ["auth","csrf"]))
            ...
            if (statuses and
                ...
                result.add(Finding(self.NAME,Severity.HIGH,
                    f"No Rate Limiting: {path}",
                    f"5 concurrent POSTs were not blocked ({statuses}).",
                    f"Responses: {statuses}",
                    "Implement rate limiting.",7.5,"CWE-307",base+path,["auth"]))
        await asyncio.gather(*[check(p) for p in self.PATHS], return_exceptions=True)

# FIX: reuse resolver + DNS rate limiting with Semaphore
class AsyncSubdomainEnum(AsyncModule):
        ...
        for target, ip in found:
            for scheme in ["https","http"]:
                ...
                if resp and resp["status"] < 500:
                    ...
                    result.add(Finding(self.NAME,sev,
                        f"Active Subdomain: {target}",
                        "Non-production subdomains are often less secure!"
                        if is_dev else "",
    ...
    async def _resolve_aiodns(self, base_d: str):
        """
        FIX: One resolver instance is reused for all queries.
        Semaphore limits concurrency to avoid DNS rate limiting.
        """
        resolver = aiodns.DNSResolver()  # single instance, reused
    ...
    async def _resolve_socket(self, base_d: str):
        ...
        Semaphore limits concurrency to avoid flooding the thread pool.
        """
        loop = asyncio.get_event_loop()
        # FIX 11: Add semaphore — same as _resolve_aiodns
        sem  = asyncio.Semaphore(self.DNS_CONCURRENCY)

        def _single_lookup(subdomain):
            # Sync OK — this function runs in the thread pool via _bounded
            try:
...
#  FIX: dangerous probes only when --aggressive is set
# ─────────────────────────────────────────────
class AsyncSSRFDetector(AsyncModule):
    NAME = "SSRF"

    # Safe probes — always executed
    SAFE_PROBES = [
    ...
    # Dangerous probes — only when --aggressive is set
    # FIX 8: Add gopher, ftp, ldap, jar for broader SSRF coverage
    AGGRESSIVE_PROBES = [
        ...
        ("file:///etc/passwd",        "File Read: /etc/passwd"),
        ("file:///etc/hostname",      "File Read: /etc/hostname"),
        ("gopher://127.0.0.1:6379/_PING%0D%0A", "Gopher Redis"),
        ("ftp://127.0.0.1/",          "FTP Loopback"),
        ("ldap://127.0.0.1/",         "LDAP Loopback"),
    ]
    INDICATORS = [
        r"ami-id", r"instance-id", r"iam/security-credentials",
        r"root:x:0:0", r"daemon:x:1:", r"computeMetadata",
        r"project-id", r"\+OK", r"220.*ftp",
    ]
    URL_KEYWORDS = [
    ...
    async def run_async(self, engine, result, url, waf_info, aggressive=False):
        ...
        if not aggressive:
            result.add(Finding(self.NAME, Severity.INFO,
                "SSRF: Safe Mode — Use --aggressive for metadata probes",
                "Dangerous probes (AWS/GCP metadata, file://) are disabled "
                "to prevent unintended SSRF against internal networks.",
                f"URL parameters detected: {[list(t[2].keys()) for t in targets[:3]]}",
                "Run with --aggressive for full probing.",
                0,"",url,["ssrf","info"]))
            ...
            for probe, label in probes[:5]:
                ...
                for pat in self.INDICATORS:
                    if re.search(pat, body, re.I|re.S):
                        result.add(Finding(self.NAME, Severity.CRITICAL,
                            f"SSRF Detected — {label}",
                            "Server made a request to an internal URL controlled by the attacker.",
                            f"URL: {t_url}\nProbe: {probe}\nMatch: {pat}\n"
                            f"Response: {body[:200]}",
                            "Implement a URL allowlist. "
                            "Block RFC-1918 and cloud metadata IPs.",
                ...
                if (r2["status"] == 200 and len(body) > 200 and
                    ...
                    result.add(Finding(self.NAME, Severity.HIGH,
                        f"Potential SSRF — {label}",
                        "Parameter accepts a URL probe — response differs significantly.",
                        f"URL: {t_url}\nProbe: {probe}\n"
                        f"Baseline: {bl_len}b → Probe: {len(body)}b",
                        "Whitelist allowed URLs.",7.5,"CWE-918",t_url,["ssrf"]))
                    return
...
class AsyncXXEDetector(AsyncModule):
            ...
            for payload, label in payloads:
                ...
                for pat in self.INDICATORS:
                    if re.search(pat, body, re.I|re.S):
                        result.add(Finding(self.NAME, Severity.CRITICAL,
                            ...
                            f"URL: {ep_url}\nPayload: {label}\n"
                            f"Response: {body[:300]}",
                            "Disable external entities. Use defusedxml.",
                ...
                if re.search(r"(xml|entity|external|DOCTYPE|parse.?error)",
                    ...
                    result.add(Finding(self.NAME, Severity.MEDIUM,
                        ...
                        "XML parser responded to an entity — possible blind XXE.",
                        f"URL: {ep_url}\nPayload: {label}\n"
                        f"Status: {r2['status']}\nSnippet: {body[:200]}",
    ...
    async def _find_xml_endpoints(self, engine, url):
        """
        FIX: Add multipart/form-data and SVG upload vectors.
        """
        endpoints = []
        base = url.rstrip("/")
...
# ─────────────────────────────────────────────
#  v7 NEW: IDOR CHECKER
#  FIX: field-level verification, not just similarity
# ─────────────────────────────────────────────
class AsyncIDORChecker(AsyncModule):
    ...
    # Regex for data that should be private
    ...
    async def _check_idor(self, engine, result, url, method, params):
        ...
        # Extract sensitive field values from the original response
        ...
        for param_key in list(params.keys())[:3]:
            ...
            for _resp_item in responses:
                ...
                # FIX: Check whether sensitive field VALUES have changed
                # (not just whether the response structure differs)
                r2_sensitive = set(self.SENSITIVE_FIELDS.findall(body2))
                new_sensitive_values = r2_sensitive - bl_sensitive

                # Same structure (sim > 0.3) but different sensitive values
                if (0.3 < sim < 0.93 and
                        len(body2) > 100 and
                        new_sensitive_values):
                    result.add(Finding(
                        self.NAME, Severity.HIGH,
                        f"IDOR Confirmed — param '{param_key}'",
                        f"ID={tid} returned sensitive data belonging to another user. "
                        "No ownership validation.",
                        f"URL: {url}\nParam: {param_key}\n"
                        f"Original ID: {original_val} → Test ID: {tid}\n"
                        f"Different sensitive data: {list(new_sensitive_values)[:3]}\n"
                        f"Similarity: {sim:.3f}",
                        "Validate ownership server-side. Use UUIDs.",
                        8.5,"CWE-639",url,["idor","auth"]))
                    return
                # Fallback: similarity only (lower confidence)
                elif (0.35 < sim < 0.90 and len(body2) > 150 and
                      abs(len(body2)-len(bl_body)) < max(len(bl_body),300)):
                    result.add(Finding(
                        self.NAME, Severity.MEDIUM,
                        f"Potential IDOR — param '{param_key}' (manual verification required)",
                        f"ID={tid} returned a different response but no sensitive "
                        "fields were detected. Manual verification required.",
                        f"URL: {url}\nParam: {param_key}\n"
                        f"Original ID: {original_val} → Test ID: {tid}\n"
                        f"Similarity: {sim:.3f}",
                        "Manual verification: check whether another user's data is accessible.",
...
class AsyncJWTAnalyzer(AsyncModule):
    NAME = "JWT Analyzer"
    DISCLAIMER = (
        "⚠️  DISCLAIMER: This tool only decodes JWT claims without "
        "verifying the signature. Findings are based on structural analysis, "
        "not proof of exploitation. Manual verification is required."
    )
    WEAK_SECRETS = [
    ...
    def _analyze_jwt(self, token: str, location: str,
        ...
        if alg in ("NONE",""):
            result.add(Finding(self.NAME, Severity.CRITICAL,
                ...
                f"JWT alg:none found. Signature is not verified.\n"
                f"{self.DISCLAIMER}",
                f"Location: {location}\nHeader: {json.dumps(header)}\n"
                f"Payload: {json.dumps(payload)[:300]}",
                "Reject alg:none. Whitelist algorithms (HS256/RS256).",
        ...
        # HMAC weak secret
            ...
            if cracked is not None:
                result.add(Finding(self.NAME, Severity.CRITICAL,
                    ...
                    f"JWT secret cracked: '{cracked}'. Tokens can be forged.\n"
                    f"{self.DISCLAIMER}",
                    f"Location: {location}\nAlgorithm: {alg}\n"
                    f"Secret: '{cracked}'\nPayload: {json.dumps(payload)[:250]}",
                    "Replace secret with a 256-bit random value. Rotate all tokens.",
                    9.8,"CWE-521",url,["jwt","weak-secret"]))
            else:
                result.add(Finding(self.NAME, Severity.INFO,
                    f"JWT HMAC Token — {alg} (secret not cracked)",
                    f"JWT HMAC token found. Secret is not in the common wordlist.\n"
                    f"{self.DISCLAIMER}",
                    f"Location: {location}\nClaims: {list(payload.keys())}",
                    "Ensure the secret is at least 256 bits and randomly generated.",0,"",url,["jwt"]))

        # RSA — algorithm confusion hint
        elif alg in ("RS256","RS384","RS512","ES256","ES384","ES512"):
            result.add(Finding(self.NAME, Severity.MEDIUM,
                f"JWT: {alg} — Check Algorithm Confusion Attack",
                f"RSA/EC JWT. Vulnerable to algorithm confusion if the server does not "
                f"strictly validate the alg claim.\n{self.DISCLAIMER}",
                f"Location: {location}\nAlg: {alg}\n"
                f"Claims: {json.dumps(payload)[:200]}",
                "Explicitly whitelist the allowed algorithm on the server.",
        ...
        if exp is None:
            result.add(Finding(self.NAME, Severity.MEDIUM,
                ...
                f"Token never expires.\n{self.DISCLAIMER}",
                f"Location: {location}\nClaims: {json.dumps(payload)[:200]}",
                "Add an 'exp' claim with a reasonable TTL.",5.4,"CWE-613",url,
                ["jwt","no-expiry"]))
        elif exp and time.time() > exp:
            result.add(Finding(self.NAME, Severity.LOW,
                "JWT: Expired Token Found",
                f"An expired token was sent to the client.\n{self.DISCLAIMER}",
                f"exp: {exp} (expired {int(time.time()-exp)}s ago)",
                "Validate exp on every request.",3.1,"CWE-613",url,["jwt"]))
        ...
        if found_s:
            result.add(Finding(self.NAME, Severity.HIGH,
                f"JWT: Sensitive Data in Payload — {found_s}",
                f"JWT payload is not encrypted (Base64 only).\n{self.DISCLAIMER}",
                f"Location: {location}\nKeys: {found_s}",
                "Do not store sensitive data in JWTs. Use JWE instead.",
        ...
        for path in self.MANIFEST_PATHS:
            ...
            if internal:
                result.add(Finding(self.NAME, Severity.HIGH,
                    ...
                    "Public manifest exposes internal package names. "
                    "Risk of supply-chain attack via public registry.",
                    f"File: {base+path}\n"
                    f"Possible internal packages: {sorted(internal)[:8]}\n"
                    f"Total: {len(packages)} packages",
                    "Use scoped packages. Private registry. Lockfile integrity.",
                    7.5,"CWE-427",base+path,["supply-chain","dep-confusion"]))
            elif packages:
                result.add(Finding(self.NAME, Severity.MEDIUM,
                    f"Public Package Manifest — {path}",
                    "Dependency list is exposed — useful for reconnaissance.",
                    f"File: {base+path}\nTotal: {len(packages)}\n"
                    f"Sample: {', '.join(list(packages)[:6])}",
                    "Restrict access to the manifest if public access is not needed.",
...
class AsyncGraphQLScanner(AsyncModule):
    """
    FIX: Detect GraphQL endpoints, attempt introspection queries,
    and fuzz basic fields if introspection is available.
    """
    NAME = "GraphQL"
    ...
    async def run_async(self, engine, result, url, waf_info, aggressive=False):
        ...
        for path in self.PATHS:
            ...
            # Attempt introspection via POST
            ...
            body = resp["body"]
            # Detect GraphQL error (endpoint exists but introspection is blocked)
            if re.search(r'"errors".*"message"', body, re.I):
                result.add(Finding(self.NAME, Severity.INFO,
                    f"GraphQL Endpoint — {path}",
                    "GraphQL found. Introspection is likely disabled.",
                    f"URL: {full}\nStatus: {resp['status']}\n"
                    f"Response: {body[:200]}",
                    "Manually verify whether introspection is enabled.",
                    0,"",full,["graphql"]))
                continue
            ...
            if '"__schema"' in body or '"types"' in body:
                ...
                # Attempt mutation fuzzing
                if aggressive:
                    await self._fuzz_mutations(engine, result, full, body)

            elif resp["status"] == 200:
                result.add(Finding(self.NAME, Severity.MEDIUM,
                    f"GraphQL Endpoint Detected — {path}",
                    "GraphQL endpoint is active.",
                    f"URL: {full}\nStatus: 200",
                    "Audit all queries and mutations.",
                    4.3,"",full,["graphql"]))

    async def _fuzz_mutations(self, engine, result, url, schema_body):
        """Basic mutation fuzzing when introspection is available."""
        mutations = []
        ...
        if mutations:
            result.add(Finding(self.NAME, Severity.MEDIUM,
                f"GraphQL Mutations Found: {mutations[:5]}",
                "Mutations exposed via introspection.",
                f"URL: {url}\nMutations: {mutations[:10]}",
                "Implement proper authorization for each mutation.",
...
def _esc(s: str) -> str:
    """
    FIX: Escape all user-supplied output to HTML to prevent XSS in the report.
    Important because findings may contain </script> tags from scan payloads.
    """
...
<div class="sec">
  <div class="disc">⚠️ <strong>DISCLAIMER</strong>: This report is for authorized security testing only.
  The JWT analyzer only decodes claims without verifying the signature.
  IDOR findings require manual verification. Unauthorized use is illegal.</div>
...
class AsyncParamGuesser:
    """
    Searches for hidden/undocumented parameters by sending a
    wordlist to an endpoint and comparing responses against the baseline.
    Heuristic: status changes or content length changes by more than 20 bytes
    → parameter has an effect → added to the SQLi/XSS/IDOR scan queue.
    """
    NAME     = "Param Guesser"
    DEFAULT_WORDLIST = [
    ...
    async def run_guess(self, base_url: str) -> list:
        """
        Scan for hidden parameters on base_url.
        Returns: list of dicts {param, url, status, length, reflected, note}
        """
        ...
        # FIX 7: Probe with several common values — harder to detect or filter
                    ...
                    try:
                        async with self.engine._session.get(
                            ...
                            if status != baseline_status or diff > 20:
                                ...
                                return  # parameter confirmed, no need to probe further
                    except Exception:
                        pass
...
def print_summary(result, ei=None):
    ...
    W2 = 72
    print(f"\n\n{'═'*W2}")
    print(bold(f"  📋  REPORT — VulnScan PRO"))
    ...
    lbl = (r("CRITICAL") if score>=70 else y("HIGH") if score>=40 else
           c("MEDIUM") if score>=20 else g("LOW"))
...
async def run_scan_async(target, args, paths, subdomains,
    ...
    # FIX 2b: --port-timeout is forwarded to the port scanner via engine attribute
    ...
    # FIX 13: Dedicated asyncio.Lock for checkpoints — prevents "database is locked"
    # when multiple modules save checkpoints simultaneously
    ...
    if not _should_skip("WAF Detector"):
        ...
        stop.set(); t.join()
        # FIX: --waf-bypass forces bypass even when no WAF is detected
    ...
    print(f"\n{p_info} Running Pre-Scan: Searching for Hidden Parameters...")
    urls_to_scan = [target]

    # FIX 4 (resume): re-populate urls_to_scan from the saved queue.
    # This ensures URLs discovered by the crawler/guesser in a previous session
    # are still processed by the SQLi/XSS/IDOR modules when the scan is resumed.
    if resume_result and result.scan_queue_urls:
        ...
        print(f"  {g('✓')}  Queue resume: {len(result.scan_queue_urls)} URL(s) "
              f"from previous session loaded into urls_to_scan.")
    ...
    if not _should_skip("Param Guesser"):
        ...
        elapsed = time.time() - t0
        if disc_params:
            print(f" {p_success} Found {len(disc_params)} potential parameter(s)!")
            for item in disc_params:
                print(f"     -> Parameter: {item['param']} | "
                      f"URL: {item['url']} "
                      f"(Reflected: {item['reflected']})")
                urls_to_scan.append(item["url"])
                # Track in scan_queue_urls so this URL survives a resume
                ...
                result.add(Finding(
                    ...
                    "Parameter affects the response — "
                    "status or length changed. Forwarded to SQLi/XSS/IDOR modules.",
                    f"URL: {item['url']}\n"
                    f"Status: {item['status']} | "
                    f"Length diff: {item['diff']} bytes | "
                    f"Reflected: {item['reflected']}",
                    "Ensure the parameter is validated and sanitized.",
                    0, "CWE-20", item["url"], ["param-guess"]))
            print(f"  [{g(f'✓ {len(disc_params)} param(s) found'):<22}] "
                  f"{bold('Param Guesser'):<42} {dim(f'{elapsed:.1f}s')}")
        else:
            print(f" {p_info} No suspicious hidden parameters detected.")
            print(f"  [{g('✓ clean'):<22}] "
                  f"{bold('Param Guesser'):<42} {dim(f'{elapsed:.1f}s')}")
    ...
    print(f"\n{p_info} Starting vulnerability scan across all target URLs...")
    ...
    for mod in core_modules:
        ...
        t0 = time.time()
        try:
            # Scan the primary target + all URLs discovered by the param guesser
        ...
        elapsed = time.time()-t0
        n = sum(1 for f in result.findings if f.module==mod.NAME)
        status = g("✓ clean") if n==0 else r(f"✗ {n} finding(s)")
        print(f"  [{status:<20}] {bold(mod.NAME):<42} "
    ...
    if use_spa and PLAYWRIGHT_OK and not _should_skip("SPA Crawler"):
        ...
        print(f"  [{dim('- skipped'):<20}] {bold('SPA Crawler'):<42} "
              f"{dim('(use --spa-crawl to enable)')}")
    elif not PLAYWRIGHT_OK:
        print(f"  [{dim('- no playwright'):<20}] {bold('SPA Crawler'):<42} "
              f"{dim('(pip install playwright)')}")

    # ── FIX 3: Second-pass Param Guesser on endpoints found by the crawler ──
    # Run the guesser on each new endpoint that contains a query string
    crawled_with_qs = [
    ...
    if crawled_with_qs and not _should_skip("Param Guesser (post-crawl)"):
        print(f"\n{p_info} Post-crawl Param Guesser on {len(crawled_with_qs)} new endpoint(s)...")
        guesser2 = AsyncParamGuesser(engine=engine)
        for ep_url in crawled_with_qs[:20]:  # cap to avoid excessive runtime
            try:
                extra = await guesser2.run_guess(ep_url)
                for item in extra:
                    if item["url"] not in urls_to_scan:
                        urls_to_scan.append(item["url"])
                        # Track in queue so it survives a resume
                        if item["url"] not in result.scan_queue_urls:
                            result.scan_queue_urls.append(item["url"])
                        ...
                        result.add(Finding(
                            ...
                            "Discovered during second-pass after crawling.",
                            f"URL: {item['url']}\nReflected: {item['reflected']}",
                            "Validate and sanitize the parameter.", 0, "CWE-20",
        ...
        await _checkpoint()
        print(f"  [{g('✓ done'):<20}] {bold('Param Guesser (post-crawl)'):<42}")
    ...
    await _checkpoint()  # Final checkpoint — also saves scan_queue_urls as pending
    # FIX 4 (mark done): mark all queue entries as done after a successful scan
    if checkpoint_db:
        try:
...
def run_demo():
    ...
    print(f"    {g('✓')} asyncio.Lock for acquire_async (replaces threading.Lock)")
    print(f"    {g('✓')} asyncio.open_connection for port scanner")
    print(f"    {g('✓')} asyncio.to_thread for SQLite I/O")
    ...
    print(f"    {g('✓')} html.escape() for all HTML report output")
    print(f"    {g('✓')} User-Agent rotation + jitter on rate limiter")
    print(f"    {g('✓')} --aggressive: gate dangerous SSRF/XXE probes")
    ...
    findings = [
        Finding("Attack Chain",Severity.CRITICAL,
            ...
            "SSRF (aggressive mode) to AWS metadata + IDOR.",
            "Chain ID: chain-006\nProbe: http://169.254.169.254/\n"
            "Response: ami-id found\nIDOR param: order_id",
            "Block metadata IPs. Use IMDSv2.",9.9,"","",["chain","chain-006"]),
        Finding("SSRF",Severity.CRITICAL,
            "SSRF Detected — AWS Metadata",
            "Parameter 'url' sends a request to the metadata endpoint.",
            f"URL: {target}/?url=http://169.254.169.254/\n"
            "Response match: ami-id",
            "URL allowlist. Block RFC-1918.",9.8,"CWE-918",
            target+"/?url=",["ssrf"]),
        Finding("XXE",Severity.CRITICAL,
            "XXE Injection — File Read (etc/passwd) [aggressive]",
            "External entity parsing is active. --aggressive mode.",
            f"URL: {target}/api/import\nPayload: file:///etc/passwd\n"
            "Response: root:x:0:0:root:/root:/bin/bash",
            "Disable external entities. Use defusedxml.",9.1,"CWE-611",
            target+"/api/import",["xxe"]),
        Finding("IDOR",Severity.HIGH,
            "IDOR Confirmed — param 'id'",
            "Sensitive field differs: another user's email is exposed.",
            f"URL: {target}/api/v1/user\nID: 1 → 2\n"
            'Sensitive data diff: ["john@example.com"]',
            "Server-side ownership validation. Use UUIDs.",8.5,"CWE-639",
            target+"/api/v1/user",["idor"]),
        Finding("JWT Analyzer",Severity.CRITICAL,
            "JWT: Weak HMAC Secret — 'secret'",
            "Secret cracked. ⚠️ DISCLAIMER: claims only, no sig verify.",
            "Algorithm: HS256\nSecret: 'secret'",
            "256-bit random secret. Rotate all tokens.",9.8,"CWE-521",
            target,["jwt","weak-secret"]),
        Finding("GraphQL",Severity.HIGH,
            "GraphQL Introspection Enabled — /graphql",
            "The entire GraphQL schema can be enumerated.",
            f"URL: {target}/graphql\nTypes found: User, Order, Admin",
            "Disable introspection in production.",7.5,"CWE-200",
        ...
        Finding("SQL Injection",Severity.CRITICAL,
            ...
            f"URL: {target}/api/v1/user\nPayload: SLEEP(2)",
            "Use Prepared Statements.",9.0,"CWE-89",
    ...
    for mod_name, delay, reqs, special in modules_sim:
        ...
        def _spin(ev=stop_ev, n=mod_name):
            ...
            while not ev.is_set():
                print(f"\r  {C.CYAN}{frames[i%10]}{C.RESET} {n:<44} "
                      f"{C.DIM}scanning...{C.RESET}",end="",flush=True)
                time.sleep(0.08); i+=1
            # FIX 15: Explicitly clear the spinner line with a fixed width
        ...
        elif nf == 0:        status = g("✓ clean")
        elif "Chain" in mod_name: status = m(f"⛓ {nf} chain!")
        else:                status = r(f"✗ {nf} finding(s)")
    ...
    print(f"\n  ✅  Open {c('vulnscan_report.html')} in your browser!\n")
...
Examples:
  python vulnscan_v7.py https://target.com
  python vulnscan_v7.py https://target.com --aggressive      # dangerous SSRF/XXE probes
  python vulnscan_v7.py https://target.com --waf-bypass      # force bypass mutation
  ...
  python vulnscan_v7.py --demo
    ...
    p.add_argument("--aggressive", action="store_true",
                   help="Enable dangerous probes: SSRF/XXE to metadata/filesystem "
                        "(opt-in, may trigger SSRF from scanner to internal network)")
    p.add_argument("--waf-bypass", action="store_true",
                   help="Force bypass mutation even when no WAF is detected")
    p.add_argument("--debug",      action="store_true",
                   help="Verbose logging + stack trace on errors")
    p.add_argument("--no-verify",  action="store_true",
                   help="Skip SSL certificate verification (default: already skipped)")
    # recent new flags
    p.add_argument("--port-timeout", type=float, default=None, metavar="SEC",
                   help="Override port scanner timeout (seconds). Default: 2.0 for common ports, 1.0 for others")
    p.add_argument("--cacert",     metavar="FILE",
                   help="Path to a custom CA bundle for SSL verification (PEM format)")
    p.add_argument("--token",      metavar="TOKEN",
                   help="Bearer token for the Authorization header (modern APIs/OAuth2)")
    return p
...
def main():
    ...
    if getattr(args,'debug',False):
        ...
        print(f"  {y('⚠')}  Debug mode active — stack traces will be shown")
    ...
    def load_wl(filepath, default, is_path=True):
        ...
        if not os.path.isfile(filepath):
            print(f"  {y('⚠')}  '{filepath}' not found — using default.")
            return default
        with open(filepath, encoding="utf-8", errors="ignore") as fp:
    ...
    if getattr(args,'resume',None):
        ...
        if resume_result:
            ...
            print(f"  {y('⚠')}  Checkpoint is empty — starting a new scan.")

    # FIX 9: Safeguard for --aggressive — prevent misuse of SSRF/file probes
    if getattr(args, 'aggressive', False):
        ...
        if is_private:
            print(f"\n  {r('⚠ WARNING')} Target appears to be a PRIVATE IP/host: {w(parsed_host)}")
            print(f"  Mode {r('--aggressive')} will send SSRF/file:// probes to the {r('internal network')}.")
            try:
                ans = input(f"  Continue? (type 'yes' to confirm): ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                ans = ""
            if ans not in ("ya", "yes", "y"):
                print(f"  {y('Cancelled.')} Remove --aggressive or use --allow-internal-probes.")
                return
        else:
            print(f"\n  {y('⚡ Aggressive mode active')} — dangerous SSRF/XXE probes enabled.")
            print(f"  {dim('Ensure you have explicit permission to test this target.')}")
    ...
    print(f"  {c('Aggressive')}  : {r('ON') if args.aggressive else dim('off (use --aggressive for dangerous SSRF/XXE probes)')}")
